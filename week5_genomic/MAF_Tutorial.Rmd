...
View(maf_object@data[TP53_Mask, c("Tumor_Sample_Barcode", "HGVSp_Short")])
```
...
ggsave("~/2023-2024 USC/Spring 2024/QBIO 490/qbio_490_justinwang/oncoplot_top10_SKCM.png")
```
...
```{r Exercise 8.2}
oncoplot(maf = maf_object,
         genes = c("TTN", "MUC16", "BRAF", "DNAH5", "PCLO"),
         borderCol = NA)
```
...
```{r Exercise 8.3}
median_age <- median(as.numeric(maf_object@clinical.data$age_at_diagnosis), na.rm = TRUE)
print(median_age)

maf_object@clinical.data$age_category <- ifelse(maf_object@clinical.data$age_at_diagnosis > median_age,'Old', 'Young')
...
```{r Exercise 9.1}
young_mask <- (maf_object@clinical.data$age_category == "Young")
...
```{r Exercise 9.3}
old_mask <- (maf_object@clinical.data$age_category == "Old")
...
ggsave("~/2023-2024 USC/Spring 2024/QBIO 490/qbio_490_justinwang/coOncoplot_GENE_GENE_SKCM.png")
```   
...
```{r Exercise 4.2}
contig <- table(clinical$vital_status, clinical$gender, exclude = "[Not Available]")
```
...
```{r}
# You have better odds of being alive if you are a woman, as compared to a man.
```


Does this make sense given the biology of cancer?
```{r}
# Not really? Maybe this has something to do with lifespans, or gender biases in 
# the data.
```
...
```{r Exercise 6.5}
num_A_only <- length(setdiff(mut_pats_geneA, mut_pats_geneB))
num_B_only <- length(setdiff(mut_pats_geneB, mut_pats_geneA))
```
...
# num_neither <- nrow(maf_object@clinical.data) - as.numeric(num_A_only) - as.numeric(num_B_only) - as.numeric(num_AB)
```
...
ggsave("~/2023-2024 USC/Spring 2024/QBIO 490/qbio_490_justinwang/lollipop_TP53_SKCM.png")
```

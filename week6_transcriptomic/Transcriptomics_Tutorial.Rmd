...
Use the dim() function to find the dimensions of the colData, rowRanges@elementMetadata, and unstranded dataframes. Record them below:
  ...
  ~ colData (rows, cols): 473 rows, 175 cols
  ~ rowRanges@elementMetadata: 60660 rows, 10 cols
...
Run the following code and fill out a description of the rows/columns of our data frames. You can use this as a cheat sheet moving forward.
  ~ colData
        - Rows: Patients
        - Columns: Clinical data (Barcode, etc.)
  ~ rowRanges@elementMetadata
        - Rows: Genes
        - Columns: Gene attributes (Ensembl ID, symbol, etc.)
  ~ unstranded (in assays)
        - Rows: Genes
        - Columns: Raw counts per patient
  
```{r Exercise 1.3}
# colData: 
head(rownames(rna_se@colData))
head(colnames(rna_se@colData))

# rowRanges:
head(rownames(rna_se@rowRanges@elementMetadata))
head(colnames(rna_se@rowRanges@elementMetadata))
...
```{r Exercise 2.1}
head(rna_se@colData$age_at_index, n = 15)
```
...
```{r Exercise 5.4}
rna_clinical$age_category <- ifelse(rna_clinical$age_at_index <= 58, 
                                    "Young",
                                    "Old")
...
head(rownames(rna_genes))
head(colnames(rna_genes))
```
...
Since there still is one patient who has "Not Reported" for vital status in our dataframes, we need to remove the patient's information form both rna_clinical and rna_counts. Use the is.na() function to accomplish this here:
...
```{r Exercise 2.6}
write.csv(rna_clinical, "/analysis_data/skcm_rna_clinical_data.csv", row.names = TRUE)
```
...
```{r Exercise 3.4}
write.csv(rna_genes, "/analysis_data/skcm_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 4.4}
write.csv(rna_counts, "/analysis_data/skcm_rna_count_data.csv", row.names = TRUE)
```

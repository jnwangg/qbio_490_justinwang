...
For this assignment, we will only be working with the base TCGA clinical data (not drug or radiation data). The following lines of code saves the base clinical data in a R dataframe called clinic. We will also rename the bcr_patient_barcode as Tumor_Sample_Barcode.
...
```{r}
write.csv(clinic, "/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/analysis_data/skcm_clinical_data.csv")
```
...
# 1. There are 470 rows and 65 columns.
# 2. $gender contains character types.
# 3. The 5th patient is 74 years old.
# 4. The 2nd patient is 82 years old ($age_at_diagnosis) and had their last follow
#    up 1856 days after diagnosis ($last_contact_days_to). They should be 87 years
#    old at this follow up.
```
...
```{r Exercise 4.1}
plot(x = clinic$age_at_diagnosis, 
     y = clinic$death_days_to, 
...
```{r Exercise 4.2}
boxplot(as.numeric(clinic$death_days_to) ~ as.factor(clinic$gender),
        main = "Days to Death vs. Gender",
        xlab = "Gender",
        ylab = "Days to Death",
        col = "#69b3a2")
```
...
```{r}
vital_status_na_mask <- !(clinic$vital_status == "[Not Available]")
```
...
```{r}
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis <= 35, 
                                      "Young",
...
```{r}
# The y-axis represents the survival probability of a patient.
```
...
```{r}
# The approx. 15-year survival rate for "Old" skin cancer patients is roughly
# 20%.
```
...
```{r}
# In this case, the p-value tests the null hypothesis that each age group has the
# same survival probability. Given our p-value of 0.0022, there is enough statistical
# significance to reject the null hypothesis. In other words, skin cancer survival
# is less likely with increasing age. 
```
